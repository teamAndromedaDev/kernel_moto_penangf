...
int get_bootargs_target(const char *target, char *value) {
	struct device_node *node = of_find_node_by_path("/chosen");
	size_t target_len = strlen(target);
	size_t bootargs_len;
	const char *bootargs;
	const char *end, *target_end, *target_begin;
	if (IS_ERR_OR_NULL(node))
		return -1;
	if(of_property_read_string(node, "bootargs", &bootargs)) {
		pr_err("%s, get prop error\n", __func__);
		return -1;
	}
	pr_info("%s: %s\n", __func__, bootargs);
	bootargs_len = strlen(bootargs);
	end = bootargs + bootargs_len;
	while((bootargs = strchr(bootargs, target[0])) && bootargs != NULL) {
		if (bootargs - 1 != end - bootargs_len && (bootargs - 1)[0] != ' ') {
			bootargs++;
			continue;
		}
		if (bootargs + target_len >= end) {
			pr_err("%s, not enough len, maybe target is blank: %s\n", __func__, bootargs);
			bootargs = end;
			break;
		}
		// pr_info("%s: %s\n", __func__, bootargs);
		if (strncmp(bootargs, target, target_len) != 0) {
			bootargs++;
			continue;
		}
		// [target_begin, target_end)
		target_begin = strchr(bootargs, '=') + 1;
		target_end = strchr(bootargs, ' ');
		if (!target_end)
			target_end = end;
		break;
	}
	if (bootargs >= end || bootargs == NULL)
		return -1;
	// pr_info("%s: %s, len: %ld\n", __func__, target_begin, target_end - target_begin);
	strncpy(value, target_begin, target_end - target_begin);
	return 0;
}
...
int ili_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	info->hwif = hwif;
	if (get_bootargs_target("lcm_index", tmp)) {
		ILI_ERR("can not parse lcm index\n");
	}
	ILI_INFO("ilitek-lcm %s\n", tmp);
	if(strcmp(tmp,"1") == 0){
		ili_tp_module = 0;
		ILI_INFO("ilitek Panle is ili_txd_interface_dev_init\n");
	} else if(strcmp(tmp,"4") == 0){
		ili_tp_module = MODEL_TM;
		ILI_INFO("ilitek Panle is ili_tm_interface_dev_init, ili_tp_module = %d\n", ili_tp_module);
	} else {
		ili_tp_module = 0;
		ILI_ERR("ilitek Panel is not ili_interface_dev_init\n");
		//return -EINVAL;
	}
