...
retry:
	...
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 start*/
	/*LOGN(tcm_hcd->pdev->dev.parent,
	...
	LOGN(tcm_hcd->pdev->dev.parent,
			"Payload length = %d\n",
			tcm_hcd->payload_length);*/
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 end*/
...
/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/03/05 start*/
...
#endif
/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/03/05 end*/
...
/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/03/05 end*/
	...
	mutex_unlock(&tcm_hcd->suspend_resume_mutex);
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 start*/
	LOGN(tcm_hcd->pdev->dev.parent, "has ovt_tcm_suspend\n");
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 end*/
	return 0;
}
#endif
...
static int ovt_tcm_early_suspend(struct device *dev)
{
	...
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 start*/
	LOGN(tcm_hcd->pdev->dev.parent, "has ovt_tcm_early_suspend\n");
	/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 end*/
	mutex_unlock(&tcm_hcd->suspend_resume_mutex);
	return 0;
}
		...
		unsigned long action, void *data)
{
	...
	struct ovt_tcm_hcd *tcm_hcd =container_of(nb, struct ovt_tcm_hcd, fb_notifier);
	...
	if (data && tcm_hcd) {
		if (atomic_read(&tcm_hcd->firmware_flashing) &&
				*transition == MTK_DISP_BLANK_POWERDOWN) {
		...
		/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 start*/
		if (action == MTK_DISP_EARLY_EVENT_BLANK && (*transition == MTK_DISP_BLANK_POWERDOWN)){
			LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_early_suspend\n");
			retval = ovt_tcm_early_suspend(&tcm_hcd->pdev->dev);
		}else if (action == MTK_DISP_EVENT_BLANK) {
			if (*transition == MTK_DISP_BLANK_POWERDOWN) {
				LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_suspend\n");
				retval = ovt_tcm_suspend(&tcm_hcd->pdev->dev);
				tcm_hcd->fb_ready = 0;
			} else if (*transition == MTK_DISP_BLANK_UNBLANK) {
#ifndef RESUME_EARLY_UNBLANK
				retval = ovt_tcm_resume(&tcm_hcd->pdev->dev);
				LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_resume\n");
				tcm_hcd->fb_ready++;
#endif
		/*penang code for EKPENAN4GU-2702 by p-zenghui at 2024/02/29 end*/
			}
		} else if (action == MTK_DISP_EARLY_EVENT_BLANK && (*transition == MTK_DISP_BLANK_UNBLANK)) {
...
/*create /sys/touchscreen */
	...
	tcm_hcd->fb_notifier.notifier_call = ovt_tcm_fb_notifier_cb;
		retval = mtk_disp_notifier_register("omnivision_touch",
				&tcm_hcd->fb_notifier);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			...
			if (ret < 0) {
				...
				if (bdatas->usb_plug_status != prop.intval) {
					LOGE(tcm_hcd->pdev->dev.parent,"usb prop.intval =%d\n",prop.intval);
					bdatas->usb_plug_status = prop.intval;
...
void ovt_tcm_charger_init(void)
{
	...
	LOGE(tcm_hcd->pdev->dev.parent,"ovt_tcm_charger_init\n");
