...
EXPORT_SYMBOL(g_capacity_level_limited_flag);
bool g_battery_protect_enable_flag = false;
EXPORT_SYMBOL(g_battery_protect_enable_flag);
int g_charger_status = 0;
EXPORT_SYMBOL(g_charger_status);
int g_quick_charging_flag = 0;
EXPORT_SYMBOL(g_quick_charging_flag);

extern int charger_set_thread_cv(u32 uV);
static DEFINE_MUTEX(pid_lock);
static int g_charger_qc_type = 0;
static struct mtk_charger *_info = NULL;

int charger_manager_set_qc_input_current_limit(int input_current)
{
	struct mtk_charger *info = _info;
...
EXPORT_SYMBOL(charger_manager_set_qc_input_current_limit);

int battery_get_vbus(void)
{
	struct mtk_charger *info = _info;
	if (info != NULL) {
		return get_vbus(info);
	}

	return 0;
}
EXPORT_SYMBOL(battery_get_vbus);

int charger_manager_set_qc_charging_current_limit(int charging_current)
{
	struct mtk_charger *info = _info;
...
EXPORT_SYMBOL(charger_manager_set_qc_charging_current_limit);

static int qc_chgstat_notify(struct mtk_charger *info)
{
	int ret = 0;
...
void hvdcp3_chgstat_notify(void)
{
	...
	if (info == NULL) {
		chr_err("mt charger info is not rdy ,skip hvdcp3\n");
		return;
	}
	...
	ret = qc_chgstat_notify(info);
	pr_info("%s uevented\n", __func__);
	if (ret)
		chr_err("%s: kobject_uevent_fail, ret=%d", __func__, ret);
	return ;
}
EXPORT_SYMBOL(hvdcp3_chgstat_notify);
...
void hvdcp_chgstat_notify(void)
{
	...
	if (info == NULL) {
		chr_err("mt charger info is not rdy ,skip hvdcp\n");
		return;
	}
	if (g_battery_protect_enable_flag) {
		chr_err("overcharge protection is on, skip hvdcp toast\n");
...
EXPORT_SYMBOL(hvdcp_chgstat_notify);
...
EXPORT_SYMBOL(charger_get_qc_type);
...
#ifndef CONFIG_CHARGER_PENANGF_DEVICE
	int vbat = 0
#endif
		...
		const char *buffer, size_t count, loff_t *data)
{
	...
	if (ret == 0) {
		...
		chr_err("%s: enable battery protect = %d\n", __func__, enable);
	} else {
...
static const struct proc_ops mtk_chg_en_battery_protect_fops = {
	.proc_open = mtk_chg_en_battery_protect_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
	.proc_write = mtk_chg_en_battery_protect_write,
};
...
static DEVICE_ATTR(Charge_Status, 0444,  Charge_Status_show, NULL);
        ...
        struct device_attribute *attr, char *buf)
{
	...
	pr_debug("[Battery] %s : %d\n",
		__func__, pinfo->chg_data[CHG1_SETTING].input_current_limit);
	return sprintf(buf, "%u\n",
		pinfo->chg_data[CHG1_SETTING].input_current_limit);
}
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		pinfo->chg_data[CHG1_SETTING].tcmd_input_current_limit = reg;

		pr_err("[Battery] %s: %d\n",
			__func__, pinfo->chg_data[CHG1_SETTING].tcmd_input_current_limit);
	}
	return size;
}
        ...
        struct device_attribute *attr, char *buf)
{
	...
	pr_debug("[Battery] %s : %d\n",
		__func__, pinfo->chg_data[CHG1_SETTING].charging_current_limit);
	return sprintf(buf, "%u\n",
		pinfo->chg_data[CHG1_SETTING].charging_current_limit);
}
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		ret = kstrtouint(buf, 10, &reg);
		pinfo->tcmd_chg_limited = 1;
		pinfo->chg_data[CHG1_SETTING].tcmd_charging_current_limit = reg;
		pr_err("[Battery] %s: %d\n", __func__,
			pinfo->chg_data[CHG1_SETTING].tcmd_charging_current_limit);
	}
	return size;
}
...
static void charger_check_status(struct mtk_charger *info)
{
	...
	if (info->capacity_level_limited && (get_uisoc(info) >= 70)) {
		charging = false;
		goto stop_charging;
	}
	/* demo version capacity lower than 65% chargind & disable limit flag */
	if ((!info->capacity_level_limited) || (get_uisoc(info) <= 65)) {
		charging = true;
	}
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d saf_cmd:%d bat_mon:%d %d cl:%d\n",
...
static void charger_status_check(struct mtk_charger *info)
{
	...
	chg_psy = power_supply_get_by_name("sgm4154x-charger");//devm_power_supply_get_by_phandle(&info->pdev->dev,"charger");
	if (IS_ERR_OR_NULL(chg_psy)) {
		chr_err("%s Couldn't get chg_psy\n", __func__);
	} else {
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < CHGS_SETTING_MAX; i++) {
		...
		info->chg_data[i].qc_charging_current_limit = -1;
		info->chg_data[i].qc_input_current_limit = -1;
	}
	info->enable_hv_charging = true;
	...
	info->chg_psy = power_supply_get_by_name("sgm4154x-charger");//devm_power_supply_get_by_phandle(&pdev->dev, "charger");
