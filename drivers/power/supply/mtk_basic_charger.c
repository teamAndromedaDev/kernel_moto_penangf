...
static void  charger_set_cv_by_ffc(struct mtk_charger *info, u32 quick_cv)
{
...
static void swchg_set_normal_charge_cv(struct mtk_charger *info)
{
    int bat_current = 0;
    int charger_qc_type;
	...
	struct chg_limit_setting *setting)
{
	...
	if (info->tcmd_chg_limited) {
		pdata->input_current_limit =
			info->chg_data[CHG1_SETTING].tcmd_input_current_limit;
		pdata->charging_current_limit =
			info->chg_data[CHG1_SETTING].tcmd_charging_current_limit;
		goto done;
	}
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		/* APPLE_CHARGER */
		if(info->usb_type == POWER_SUPPLY_USB_TYPE_ACA) {
			pdata->input_current_limit =
				info->data.apple_2_1a_charger_current;
			pdata->charging_current_limit =
				info->data.apple_2_1a_charger_current;
		} else if (info->usb_type == POWER_SUPPLY_USB_TYPE_APPLE_BRICK_ID) {
			pdata->input_current_limit =
	...
	if (pdata->thermal_charging_current_limit != -1 && pdata->qc_charging_current_limit != -1) {
		pdata->charging_current_limit = min(pdata->charging_current_limit, pdata->thermal_charging_current_limit);
		pdata->charging_current_limit = min(pdata->charging_current_limit, pdata->qc_charging_current_limit);
		info->setting.charging_current_limit1 = pdata->charging_current_limit;
		pdata->thermal_throttle_record = true;
	} else if (pdata->qc_charging_current_limit != -1) {
		pdata->charging_current_limit = min(pdata->charging_current_limit, pdata->qc_charging_current_limit);
		info->setting.charging_current_limit1 = pdata->charging_current_limit;
		pdata->thermal_throttle_record = true;
	} else if (pdata->thermal_charging_current_limit != -1) {
		pdata->charging_current_limit = min(pdata->charging_current_limit, pdata->thermal_charging_current_limit);
		info->setting.charging_current_limit1 = pdata->charging_current_limit;
		pdata->thermal_throttle_record = true;
	} else
	...
	if (pdata->thermal_input_current_limit != -1 && pdata->qc_input_current_limit != -1) {
		pdata->input_current_limit = min(pdata->input_current_limit, pdata->thermal_input_current_limit);
		pdata->input_current_limit = min(pdata->input_current_limit, pdata->qc_input_current_limit);
		info->setting.input_current_limit1 = pdata->input_current_limit;
		pdata->thermal_throttle_record = true;
	} else if (pdata->qc_input_current_limit != -1) {
		pdata->input_current_limit = min(pdata->input_current_limit, pdata->qc_input_current_limit);
		info->setting.input_current_limit1 = pdata->input_current_limit;
		pdata->thermal_throttle_record = true;
	} else if (pdata->thermal_input_current_limit != -1) {
		pdata->input_current_limit = min(pdata->input_current_limit, pdata->thermal_input_current_limit);
		info->setting.input_current_limit1 = pdata->input_current_limit;
		pdata->thermal_throttle_record = true;
	}else
	...
	chr_err("m:%d chg1:%d,%d,%d,%d,%d,%d chg2:%d,%d,%d,%d dvchg1:%d sc:%d %d %d type:%d:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d atm:%d bm:%d b:%d\n",
		...
		_uA_to_mA(pdata->qc_input_current_limit),
		_uA_to_mA(pdata->qc_charging_current_limit),
