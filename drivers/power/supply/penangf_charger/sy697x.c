				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		else if (!state.chrg_stat || !sy->charge_enabled)
		...
		case SY697X_NON_STANDARD:
			...
			if (!ret) {
				...
				case SY697x_APPLE_1A:
					sy->psy_usb_type = POWER_SUPPLY_USB_TYPE_APPLE_BRICK_ID;
...
static irqreturn_t sy697x_irq_handler_thread(int irq, void *private)
{
	...
	if (!ret) {
		if (!state) {
			charger_detect_work_func(&sy->charge_detect_delayed_work.work);
		} else {
				...
				const struct i2c_device_id *id)
{
	...
	schedule_delayed_work(&sy->charge_detect_delayed_work, 10*HZ);
