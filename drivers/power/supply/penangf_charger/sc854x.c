...
#define sc_err(fmt, ...)                                                       \
	do {                                                                   \
		if (sc->mode == STANDALONE)                                    \
			printk(KERN_ERR "[sc854x-STANDALONE]:%s:" fmt,         \
			       __func__, ##__VA_ARGS__);                       \
		else if (sc->mode == MASTER)                                   \
			printk(KERN_ERR "[sc854x-MASTER]:%s:" fmt, __func__,   \
			       ##__VA_ARGS__);                                 \
		else                                                           \
			printk(KERN_ERR "[sc854x-SLAVE]:%s:" fmt, __func__,    \
			       ##__VA_ARGS__);                                 \
	} while (0);
...
#define sc_info(fmt, ...)                                                      \
	do {                                                                   \
		if (sc->mode == STANDALONE)                                    \
			printk(KERN_INFO "[sc854x-STANDALONE]:%s:" fmt,        \
			       __func__, ##__VA_ARGS__);                       \
		else if (sc->mode == MASTER)                                   \
			printk(KERN_INFO "[sc854x-MASTER]:%s:" fmt, __func__,  \
			       ##__VA_ARGS__);                                 \
		else                                                           \
			printk(KERN_INFO "[sc854x-SLAVE]:%s:" fmt, __func__,   \
			       ##__VA_ARGS__);                                 \
	} while (0);
...
#define sc_dbg(fmt, ...)                                                       \
	do {                                                                   \
		if (sc->mode == STANDALONE)                                    \
			printk(KERN_DEBUG "[sc854x-STANDALONE]:%s:" fmt,       \
			       __func__, ##__VA_ARGS__);                       \
		else if (sc->mode == MASTER)                                   \
			printk(KERN_DEBUG "[sc854x-MASTER]:%s:" fmt, __func__, \
			       ##__VA_ARGS__);                                 \
		else                                                           \
			printk(KERN_DEBUG "[sc854x-MASTER]:%s:" fmt, __func__, \
			       ##__VA_ARGS__);                                 \
...
static int sc854x_update_bits(struct sc854x *sc, u8 reg, u8 mask, u8 data)
{
	int ret;
	u8 tmp;
...
static int sc854x_enable_batovp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_00, SC854X_BAT_OVP_DIS_MASK,
				  val);
}
...
static int sc854x_set_batovp_th(struct sc854x *sc, int threshold)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_00, SC854X_BAT_OVP_MASK, val);
}
...
static int sc854x_enable_batocp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_01, SC854X_BAT_OCP_DIS_MASK,
				  val);
}
...
static int sc854x_set_batocp_th(struct sc854x *sc, int threshold)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_01, SC854X_BAT_OCP_MASK, val);
}
...
static int sc854x_set_acovp_th(struct sc854x *sc, int threshold)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_02, SC854X_AC_OVP_MASK, val);
}
...
static int sc854x_enable_dropovp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_03, SC854X_VDROP_OVP_DIS_MASK,
				  val);
}
...
static int sc854x_enable_busovp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_04, SC854X_VBUS_OVP_DIS_MASK,
				  val);
}
...
static int sc854x_set_busovp_th(struct sc854x *sc, int threshold)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_04, SC854X_VBUS_OVP_MASK, val);
}
...
static int sc854x_enable_busucp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_05, SC854X_IBUS_UCP_DIS_MASK,
				  val);
}
...
static int sc854x_enable_busocp(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_05, SC854X_IBUS_OCP_DIS_MASK,
				  val);
}
...
static int sc854x_set_busocp_th(struct sc854x *sc, int threshold)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_05, SC854X_IBUS_OCP_MASK, val);
}
...
static int sc854x_enable_charge(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_07, SC854X_CHG_EN_MASK, val);
}
...
static int sc854x_reg_reset(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_07, SC854X_REG_RESET_MASK,
				  val);
}
...
static int sc854x_set_ss_timeout(struct sc854x *sc, int timeout)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_08, SC854X_SS_TIMEOUT_SET_MASK,
				  val);
}
...
static int sc854x_voutovp_enabled(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_08, SC854X_VOUT_OVP_DIS_MASK,
				  val);
}
...
static int sc854x_set_sense_resistor(struct sc854x *sc, int r_mohm)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_08,
				  SC854X_SET_IBAT_SNS_RES_MASK, val);
}
...
static int sc854x_vbus_pd_enabled(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_08, SC854X_VBUS_PD_EN_MASK,
				  val);
}
...
static int sc854x_set_charge_mode(struct sc854x *sc, u8 charge_mode)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_09, SC854X_CHARGE_MODE_MASK,
				  val);
}
...
static int sc854x_get_charge_mode(struct sc854x *sc, int *result)
{
	...
	*result = (val >> SC854X_CHARGE_MODE_SHIFT) & SC854X_CHARGE_MODE_MASK;
...
static int sc854x_set_wdt(struct sc854x *sc, int ms)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_09, SC854X_WATCHDOG_MASK, val);
}
...
static int sc854x_vbat_regulation_enabled(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0A, SC854X_VBAT_REG_EN_MASK,
				  val);
}
...
static int sc854x_set_vbat_regulation(struct sc854x *sc, int mv)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0A, SC854X_SET_VBATREG_MASK,
				  val);
}
...
static int sc854x_ibat_regulation_enabled(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0B, SC854X_IBAT_REG_EN_MASK,
				  val);
}
...
static int sc854x_set_ibat_regulation(struct sc854x *sc, int ma)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0B, SC854X_SET_VBATREG_MASK,
				  val);
}
...
static int sc854x_ibus_regulation_enabled(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0C, SC854X_IBUS_REG_EN_MASK,
				  val);
}
...
static int sc854x_set_pmid2out_uvp(struct sc854x *sc, int mv)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0D, SC854X_PMID2OUT_UVP_MASK,
				  val);
}
...
static int sc854x_set_pmid2out_ovp(struct sc854x *sc, int mv)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_0D, SC854X_PMID2OUT_OVP_MASK,
				  val);
}
...
static int sc854x_enable_adc(struct sc854x *sc, bool enable)
{
	...
	return sc854x_update_bits(sc, SC854X_REG_11, SC854X_ADC_EN_MASK, val);
}
...
{
	return sc854x_update_bits(
		sc, SC854X_REG_3C, SC854X_VBUS_IN_RANGE_DIS_MASK,
...
static int sc854x_set_work_mode(struct sc854x *sc, int mode)
{
	...
	sc_err("[%s] work mode is %s\n",
	       mode <= SC8545_SLAVE ?
		       ...
		       (mode <= SC8546_SLAVE ?
				"SC8546" :
				(mode <= SC8547_SLAVE ? "SC8547" : "SC8549")),
	       sc->mode == STANDALONE ?
		       "standalone" :
		       (sc->mode == MASTER ? "master" : "slave"));
...
static int sc854x_parse_dt(struct sc854x *sc, struct device *dev)
{
	...
	sc->cfg = devm_kzalloc(dev, sizeof(struct sc854x_cfg), GFP_KERNEL);

	if (!sc->cfg)
		return -ENOMEM;

	sc->cfg->bat_ovp_disable =
		of_property_read_bool(np, "sc,sc854x,bat-ovp-disable");
	sc->cfg->bat_ocp_disable =
		of_property_read_bool(np, "sc,sc854x,bat-ocp-disable");
	sc->cfg->vdrop_ovp_disable =
		of_property_read_bool(np, "sc,sc854x,vdrop-ovp-disable");
	sc->cfg->bus_ovp_disable =
		of_property_read_bool(np, "sc,sc854x,bus-ovp-disable");
	sc->cfg->bus_ucp_disable =
		of_property_read_bool(np, "sc,sc854x,bus-ucp-disable");
	sc->cfg->bus_ocp_disable =
		of_property_read_bool(np, "sc,sc854x,bus-ocp-disable");
...
static DEVICE_ATTR(registers, 0660, sc854x_show_registers,
		   sc854x_store_register);
...
static enum power_supply_property sc854x_charger_props[] = {
	...
	POWER_SUPPLY_PROP_CAPACITY_ERROR_MARGIN,//POWER_SUPPLY_PROP_CAPACITY_ERROR_MARGIN,
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		if (!ret) {
			sc->batt_present =
				!!(reg_val & SC854X_VBAT_INSERT_STAT_MASK);
	...
	case POWER_SUPPLY_PROP_POWER_NOW:
		...
		if (!ret) {
			sc->vbus_present =
				!!(reg_val & SC854X_ADAPTER_INSERT_STAT_MASK);
...
static int sc854x_irq_register(struct sc854x *sc)
{
	...
	if (gpio_is_valid(sc->irq_gpio)) {
		ret = gpio_request_one(sc->irq_gpio, GPIOF_DIR_IN,
				       "sc854x_irq");
		if (ret) {
	...
	if (sc->irq) {
		if (sc->mode == STANDALONE) {
			ret = devm_request_threaded_irq(
				&sc->client->dev, sc->irq, NULL,
				sc854x_charger_interrupt,
