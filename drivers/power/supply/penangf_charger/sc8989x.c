...
static int sc8989x_enable_otg(struct regulator_dev *rdev)
{
	...
	val = SC8989X_OTG_ENABLE << SC8989X_OTG_CONFIG_SHIFT;

	return sc8989x_update_bits(sc, SC8989X_REG_03, SC8989X_OTG_CONFIG_MASK,
				   val);
}
...
static int sc8989x_set_vsys_min(struct sc8989x *sc, int val)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_03, SC8989X_SYS_MINV_MASK,
				   limit);
}
...
static int sc8989x_enable_hvdcp(struct sc8989x *sc)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_02, SC8989X_HVDCPEN_MASK,
				  val);
	return ret;
}
...
static int sc8989x_disable_charger(struct sc8989x *sc)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_03, SC8989X_CHG_CONFIG_MASK,
				  val);
	sc->charge_enabled = false;
	return ret;
}
...
int sc8989x_adc_start(struct sc8989x *sc, bool oneshot)
{
	...
	if (oneshot)
		ret = sc8989x_update_bits(
			sc, SC8989X_REG_02, SC8989X_CONV_START_MASK,
			SC8989X_CONV_START << SC8989X_CONV_START_SHIFT);
	else
		ret = sc8989x_update_bits(
			sc, SC8989X_REG_02, SC8989X_CONV_RATE_MASK,
...
int sc8989x_adc_stop(struct sc8989x *sc)
{
	return sc8989x_update_bits(sc, SC8989X_REG_02, SC8989X_CONV_RATE_MASK,
				   SC8989X_ADC_CONTINUE_DISABLE
					   << SC8989X_CONV_RATE_SHIFT);
}
...
int sc8989x_adc_read_battery_volt(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		volt = SC8989X_BATV_BASE +
		       ((val & SC8989X_BATV_MASK) >> SC8989X_BATV_SHIFT) *
			       SC8989X_BATV_LSB;
		return volt;
	}
}
...
int sc8989x_adc_read_sys_volt(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		volt = SC8989X_SYSV_BASE +
		       ((val & SC8989X_SYSV_MASK) >> SC8989X_SYSV_SHIFT) *
			       SC8989X_SYSV_LSB;
		return volt;
	}
}
...
int sc8989x_adc_read_vbus_volt(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		volt = SC8989X_VBUSV_BASE +
		       ((val & SC8989X_VBUSV_MASK) >> SC8989X_VBUSV_SHIFT) *
			       SC8989X_VBUSV_LSB;
		return volt;
	}
}
...
int sc8989x_adc_read_temperature(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		temp = SC8989X_TSPCT_BASE +
		       ((val & SC8989X_TSPCT_MASK) >> SC8989X_TSPCT_SHIFT) *
			       SC8989X_TSPCT_LSB;
		return temp;
	}
}
...
int sc8989x_adc_read_charge_current(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		volt = (int)(SC8989X_ICHGR_BASE + ((val & SC8989X_ICHGR_MASK) >>
						   SC8989X_ICHGR_SHIFT) *
							  SC8989X_ICHGR_LSB);
		return volt;
	}
...
int sc8989x_set_chargecurrent(struct sc8989x *sc, int curr)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_04, SC8989X_ICHG_MASK,
				   ichg << SC8989X_ICHG_SHIFT);
}
...
int sc8989x_set_term_current(struct sc8989x *sc, int curr)
{
	...
	iterm = (curr + 29) /
		SC8989X_ITERM_LSB; // + 29 in order to seam protection

	return sc8989x_update_bits(sc, SC8989X_REG_05, SC8989X_ITERM_MASK,
				   iterm << SC8989X_ITERM_SHIFT);
}
...
int sc8989x_set_prechg_current(struct sc8989x *sc, int curr)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_05, SC8989X_IPRECHG_MASK,
				   iprechg << SC8989X_IPRECHG_SHIFT);
}
...
int sc8989x_set_chargevolt(struct sc8989x *sc, int volt)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_06, SC8989X_VREG_MASK,
				   val << SC8989X_VREG_SHIFT);
}

int sc8989x_set_vindpm_offset_os(struct sc8989x *sc, u8 offset_os)
{
	int ret;
	ret = sc8989x_update_bits(sc, SC8989X_REG_0D, SC8989X_FORCE_VINDPM_MASK,
				  offset_os);
	if (ret) {
...
int sc8989x_set_input_volt_limit(struct sc8989x *sc, int volt)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_0D, SC8989X_VINDPM_MASK,
				   val << SC8989X_VINDPM_SHIFT);
}
...
int sc8989x_set_watchdog_timer(struct sc8989x *sc, u8 timeout)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_07, SC8989X_WDT_MASK, val);
}
...
int sc8989x_reset_watchdog_timer(struct sc8989x *sc)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_03, SC8989X_WDT_RESET_MASK,
				   val);
}
...
int sc8989x_force_dpdm(struct sc8989x *sc)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_02, SC8989X_FORCE_DPDM_MASK,
				  val);
...
int sc8989x_reset_chip(struct sc8989x *sc)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_14, SC8989X_RESET_MASK, val);
	return ret;
}
...
int sc8989x_enter_hiz_mode(struct sc8989x *sc)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_00, SC8989X_ENHIZ_MASK, val);
}
...
static int sc8989x_enable_ilmt(struct sc8989x *sc, bool enable)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_00, SC8989X_ENILIM_MASK, val);
...
static int sc8989x_enable_term(struct sc8989x *sc, bool enable)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_07, SC8989X_EN_TERM_MASK,
				  val);
...
int sc8989x_set_boost_current(struct sc8989x *sc, int curr)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_0A, SC8989X_BOOST_LIM_MASK,
...
static int sc8989x_enable_auto_dpdm(struct sc8989x *sc, bool enable)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_02, SC8989X_AUTO_DPDM_EN_MASK,
				  val);
...
int sc8989x_set_boost_voltage(struct sc8989x *sc, int volt)
{
	...
	return sc8989x_update_bits(sc, SC8989X_REG_0A, SC8989X_BOOSTV_MASK,
				   val);
}
...
static int sc8989x_read_idpm_limit(struct sc8989x *sc)
{
	...
	if (ret < 0) {
		...
		curr = SC8989X_IDPM_LIM_BASE + ((val & SC8989X_IDPM_LIM_MASK) >>
						SC8989X_IDPM_LIM_SHIFT) *
						       SC8989X_IDPM_LIM_LSB;
		return curr;
	}
}
...
static int sc8989x_enable_apsd_bc12(struct sc8989x *sc)
{
	...
	ret = sc8989x_update_bits(sc, SC8989X_REG_02, SC8989X_CHG_DPDM_MASK,
				  val);
						      ...
						      struct sc8989x *sc)
{
	...
	if (ret) {
		pdata->iterm = 180;
		pr_err("Failed to read node of sc,sc8989x,term-current\n");
	}
...
static int sc8989x_get_state(struct sc8989x *sc, struct sc8989x_state *state)
{
	u8 chrg_stat;
	u8 fault;
	...
	if (ret) {
		...
		if (ret) {
			pr_err("%s chrg_type =%d,chrg_stat =%d online = %d\n",
			       __func__, sc->state.chrg_type,
			       sc->state.chrg_stat, sc->state.online);
	...
	state->chrg_stat =
		(chrg_stat & SC8989X_CHRG_STAT_MASK) >> SC8989X_CHRG_STAT_SHIFT;
	...
	if (state->chrg_stat == 0 ||
	    state->chrg_stat == SC8989X_CHRG_STAT_CHGDONE) {
		state->chrg_en = false;
	} else {
		state->chrg_en = true;
	}
...
/*Penang code for EKPENAN4GU-3593 by zhoujiangjiang at 2024/4/22 start*/
	schedule_delayed_work(&sc->charge_detect_delayed_work, 100);
/*Penang code for EKPENAN4GU-3593 by zhoujiangjiang at 2024/4/22 end*/
...
static void sc8989x_dump_regs(struct sc8989x *sc)
{
	unsigned char i = 0;
	unsigned int ret = 0;
	unsigned char sc8989x_reg[SC8989X_REG_14+1] = { 0 };
	char buffer[1024] = {0};
	char *ptr = buffer;


	for (i = 0; i < SC8989X_REG_14 + 1; i++) {
		ret = sc8989x_read_byte(sc, i, &sc8989x_reg[i]);
		if (ret == 0)
...
static ssize_t sc8989x_store_registers(struct device *dev,
				       ...
				       const char *buf, size_t count)
{
	...
	if (ret == 2 && reg < 0x14) {
		sc8989x_write_byte(sc, (unsigned char)reg, (unsigned char)val);
	}
...
static int32_t sc8989x_set_dpdm(struct sc8989x *sc, uint8_t dp_val,
				uint8_t dm_val)
{
	uint8_t data_reg = 0;
...
{
	struct sc8989x *sc =
		container_of(work, struct sc8989x, hvdcp_work.work);
	int vbus;
	int i;
...
static void set_iindpm_vindpm_intput_disable(struct sc8989x *sc)
{
	...
	sc8989x_update_bits(sc, SC8989X_REG_88, SC8989X_VINDPM_INT_MASK_MASK,
			    SC8989X_VINDPM_INT_MASK
				    << SC8989X_VINDPM_INT_MASK_SHIFT);
	sc8989x_update_bits(sc, SC8989X_REG_88, SC8989X_IINDPM_INT_MASK_MASK,
			    SC8989X_IINDPM_INT_MASK
				    << SC8989X_IINDPM_INT_MASK_SHIFT);
	sc8989x_set_key(sc);
}
		...
		if (!ret) {
			if (state) {
				...
				schedule_delayed_work(
					&sc->charge_detect_delayed_work, 100);
...
static void charger_monitor_work_func(struct work_struct *work)
{
	...
	sc = container_of(charge_monitor_work, struct sc8989x,
			  charge_monitor_work);
	if (sc == NULL) {
		pr_err("Cann't get sc \n");
		return;
...
static void charger_detect_work_func(struct work_struct *work)
{
	...
	sc = container_of(charge_detect_delayed_work, struct sc8989x,
			  charge_detect_delayed_work);
	if (sc == NULL) {
	...
	case SC8989x_USB_HVDCP:
		sc8989x_power_supply_desc.type = POWER_SUPPLY_TYPE_USB_DCP;   //QC
		sc->psy_usb_type = POWER_SUPPLY_USB_TYPE_DCP;
		chrg_type = STANDARD_CHARGER;
		...
		schedule_delayed_work(
			&sc->hvdcp_work,
			msecs_to_jiffies(1400)); //needs delay 1.4S
		g_hvdcp_type = true;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		else if (!state.chrg_stat || !sc->charge_enabled)
...
static int sc8989x_power_supply_init(struct sc8989x *sc, struct device *dev)
{
	...
	sc->charger = devm_power_supply_register(
		sc->dev, &sc8989x_power_supply_desc, &psy_cfg);
...
static int sc8989x_vbus_regulator_register(struct sc8989x *sc)
{
	...
	sc->otg_rdev =
		devm_regulator_register(sc->dev, &sc8989x_otg_rdesc, &config);
				 ...
				 const struct i2c_device_id *id)
{
	...
	sc->chg_dev =
		charger_device_register(sc->chg_dev_name, &client->dev, sc,
					&sc8989x_chg_ops, &sc8989x_chg_props);
	...
	schedule_delayed_work(&sc->charge_detect_delayed_work, 10*HZ);
