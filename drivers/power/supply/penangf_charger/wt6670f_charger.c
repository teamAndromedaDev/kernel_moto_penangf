...
EXPORT_SYMBOL_GPL(g_qc3_type_set);
u16 g_qc3p_id = 0;
...
static int wt6670f_i2c_sequence_send(const struct i2c_client *client,
				     const char *buf, int count)
{
	...
	msg[0].flags = 0x0000;
	msg[0].flags = msg[0].flags | I2C_M_NO_RD_ACK | I2C_M_IGNORE_NAK |
		       I2C_M_NOSTART;
	msg[0].len = 1;
	msg[0].buf = (char *)buf;
	...
	msg[1].flags = 0x0000;
	msg[1].flags = msg[1].flags | I2C_M_NO_RD_ACK | I2C_M_IGNORE_NAK |
		       I2C_M_NOSTART;
	msg[1].len = 1;
	msg[1].buf = (char *)buf;
...
EXPORT_SYMBOL_GPL(wt6670f_charger_reset_reg);
...
static int wt6670f_charger_type(struct wt6670f_charger *chip)
{
	...
	if ((data1 > 0x00 && data1 < 0x07) || (data1 > 0x07 && data1 < 0x0a) ||
	    (QC3P_Z350 == g_qc3p_id && data1 == 0x10)) {
		ret = data1;
	} else {
		ret = 0x00;
	}
...
void wt6670f_get_charger_type_func_work(struct work_struct *work)
{
	...
	if (QC3P_WT6670F == g_qc3p_id) {
		do {
			...
			while ((!m_chg_ready) && (count < 13)) {
				...
				if (!early_notified) {
					early_chg_type =
						wt6670f_charger_type(chip);
				}

				if (early_chg_type == 0x08 ||
				    early_chg_type == 0x09) {
				...
				pr_info("wt6670f waiting early type:0x%x, detect ready: 0x%x",
					early_chg_type, m_chg_ready);
	...
	if (QC3P_Z350 == g_qc3p_id) { //z350
		...
		if ((m_chg_type != 0x02) && (m_chg_type != 0x03)) {
			if (!chip->charger_psy)
				chip->charger_psy = power_supply_get_by_name(
					"sgm4154x-charger");
	...
	if (QC3P_Z350 == g_qc3p_id)
		...
		if (m_chg_type == 0x08 || m_chg_type == 0x09) {  //if (m_chg_type == 0x08 && m_chg_type == 0x09) {
			g_change_num++;
...
EXPORT_SYMBOL_GPL(wt6670f_get_charger_type_func_work);

int wt6670f_i2c_read_cmd(const struct i2c_client *client, char read_addr,
			 const char *buf)
{
	int ret;
	struct i2c_adapter *adap = client->adapter;
...
int wt6670f_i2c_master_send(const struct i2c_client *client, const char *buf,
			    int count)
{
	int ret;
	struct i2c_adapter *adap = client->adapter;
...
void wt6670f_isp_flow(struct wt6670f_charger *chip)
{
	...
	rc = wt6670f_i2c_master_send(chip->client, enable_isp_cmd,
				     ENABLE_ISP_CMD_LEN);
	...
	if (chip_id == ISP_CHIP_ID) {
		rc = wt6670f_i2c_master_send(chip->client,
					     enable_isp_flash_mode_cmd,
					     I2C_MASTER_SEND_LEN);
		dev_err(chip->dev, "%s: enable_isp_flash_mode_cmd, rc=%d\n",
			__func__, rc);

		rc = wt6670f_i2c_master_send(chip->client, chip_erase_cmd,
					     I2C_MASTER_SEND_LEN);
		if (rc != I2C_MASTER_SEND_LEN) {
			dev_err(chip->dev,
				"%s: chip_erase_cmd is failed, rc=%d\n",
				__func__, rc);
		...
		rc = wt6670f_i2c_master_send(chip->client, finish_cmd,
					     I2C_MASTER_SEND_LEN);
		if (rc != I2C_MASTER_SEND_LEN) {
		...
		while (pos < size) {
			...
			length = FIRWARE_SIZE;
			dev_err(chip->dev,
				"%s: high_addr = %02x, low_addr = %02x, length=%d\n",
				__func__, high_addr, low_addr, length);

			memset(code, 0, FIRWARE_SIZE);
			set_addr_high_byte_cmd[2] = high_addr;
			rc = wt6670f_i2c_master_send(chip->client,
						     set_addr_high_byte_cmd, 3);
			if (rc != 3) {
				dev_err(chip->dev,
					"%s: set_addr_high_byte_cmd is failed, rc=%d\n",
					__func__, rc);
			...
			rc = wt6670f_i2c_master_send(chip->client, program_cmd,
						     FIRWARE_SIZE + 2);
			if (rc != FIRWARE_SIZE + 2) {
				dev_err(chip->dev,
					"%s: program_cmd is failed, rc=%d\n",
					__func__, rc);
		...
		for (i = 0; i < 16; i++) {
			...
			flash_addr = 0x00;
			wt6670f_i2c_read_cmd(chip->client, flash_addr,
					     mem_data);
			for (j = 0; j < 64; j++) {
				dev_err(chip->dev, "%s: mem_data[%d]=%02x\n",
					__func__, j + flash_addr + i * 256,
					mem_data[j]);
				dev_err(chip->dev,
					"%s: wt6670f_qc3p_bin[%d]=%02x\n",
					__func__, j + flash_addr + i * 256,
					wt6670f_qc3p_bin[j + flash_addr +
							 i * 256]);
				if (wt6670f_qc3p_bin[j + flash_addr + i * 256] !=
				    mem_data[j]) {
					dev_err(chip->dev,
						"%s: flash data is wrong.\n",
						__func__);
		...
		dev_err(chip->dev,
			"%s: chip id is not right, and end update.\n",
			__func__);
...
static int wt6670f_parse_dt(struct wt6670f_charger *chip)
{
	...
	chip->wt6670f_sda_gpio =
		of_get_named_gpio(np, "wt,wt6670f_sda_gpio", 0);
	...
	chip->wt6670f_scl_gpio =
		of_get_named_gpio(np, "wt,wt6670f_scl_gpio", 0);
	...
	chip->bc12_unsupported =
		of_property_read_bool(np, "wt,bc12_unsupported");
	chip->hvdcp_unsupported =
		of_property_read_bool(np, "wt,hvdcp_unsupported");
	chip->intb_unsupported =
		of_property_read_bool(np, "wt,intb_unsupported");
	chip->sleep_unsupported =
		of_property_read_bool(np, "wt,sleep_unsupported");
...
static int isp_pinctrl_enable(struct wt6670f_charger *chip)
{
	...
	int rc = 0;
	u16 version = 0;
	char i2c_buf_enable_cmd[7] = {
		0x57, 0x54, 0x36, 0x36, 0x37, 0x30, 0x46
	};
	...
	if ((rc) || (version != WT6670F_FIRMWARE_VERSION)) {
		...
		rc = devm_gpio_request(chip->dev, chip->wt6670f_sda_gpio,
				       "wt6670f_sda_gpio");
		if (rc) {
			pr_err("request wt6670f_sda_gpio failed, rc=%d\n", rc);
			goto error;
		}
		rc = devm_gpio_request(chip->dev, chip->wt6670f_scl_gpio,
				       "wt6670f_scl_gpio");
		if (rc) {
		...
		devm_gpio_free(chip->dev, chip->wt6670f_sda_gpio);
		devm_gpio_free(chip->dev, chip->wt6670f_scl_gpio);
		dev_err(chip->dev,
			"%s: step 2.1: select pinstate normal to i2c.\n",
			__func__);
		rc = pinctrl_select_state(chip->wt6670f_pinctrl,
		...
		rc = wt6670f_i2c_master_send(chip->client, i2c_buf_enable_cmd,
					     7);
...
static int wt6670f_pinctrl_init(struct wt6670f_charger *chip)
{
	...
	chip->pinctrl_state_normal =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "rst_normal");
	...
	chip->pinctrl_state_isp =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "rst_isp");
	if (IS_ERR_OR_NULL(chip->pinctrl_state_isp)) {
		ret = PTR_ERR(chip->pinctrl_state_isp);
		dev_err(chip->dev, "Can not lookup wt6670f_isp  pinstate %d\n",
			ret);
		goto err_pinctrl_lookup;
	}

	chip->pinctrl_scl_normal =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "scl_normal");
	if (IS_ERR_OR_NULL(chip->pinctrl_scl_normal)) {
		ret = PTR_ERR(chip->pinctrl_scl_normal);
		dev_err(chip->dev, "Can not lookup scl_normal pinstate %d\n",
			ret);
		goto err_pinctrl_lookup;
	}

	chip->pinctrl_scl_isp =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "scl_isp");
	if (IS_ERR_OR_NULL(chip->pinctrl_scl_isp)) {
		ret = PTR_ERR(chip->pinctrl_scl_isp);
		dev_err(chip->dev, "Can not lookup scl_isp  pinstate %d\n",
			ret);
		goto err_pinctrl_lookup;
	}

	chip->pinctrl_sda_normal =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "sda_normal");
	if (IS_ERR_OR_NULL(chip->pinctrl_sda_normal)) {
		ret = PTR_ERR(chip->pinctrl_sda_normal);
		dev_err(chip->dev, "Can not lookup sda_normal pinstate %d\n",
			ret);
		goto err_pinctrl_lookup;
	}

	chip->pinctrl_sda_isp =
		pinctrl_lookup_state(chip->wt6670f_pinctrl, "sda_isp");
	if (IS_ERR_OR_NULL(chip->pinctrl_sda_isp)) {
		ret = PTR_ERR(chip->pinctrl_sda_isp);
		dev_err(chip->dev, "Can not lookup sda_isp  pinstate %d\n",
			ret);
		goto err_pinctrl_lookup;
	}
...
static const char * const QC30_DEVICES[] = {
	"QC3P_Z350", "QC3P_WT6670F"
};

static ssize_t show_qc30_device(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int count = 0;

	count += sprintf(buf + count, "%s ",
				QC30_DEVICES[g_qc3p_id]);
	if (count)
		buf[count - 1] = '\n';
	return count;
}
static DEVICE_ATTR(qc30_device, 0440, show_qc30_device, NULL);
				 ...
				 const struct i2c_device_id *id)
{
	...
	rc = devm_gpio_request(&client->dev, chip->rst_gpio,
			       "wt6670f reset gpio");
	if (rc) {
	...
	rc = devm_gpio_request(&client->dev, chip->int_gpio,
			       "wt6670f int gpio");
	if (rc) {
	...
	// chip->chg_consumer =
	// 	charger_manager_get_by_name(chip->dev, "primary_chg");
	// if (!chip->chg_consumer) {
	// 	pr_err("%s: get chg_consumer failed\n", __func__);
	// }

	/*Penang code for EKPENAN4GU-2948 by hq_zhangjiangbin_tmp at 2024/04/08 start*/
	_chip = chip;
	/*Penang code for EKPENAN4GU-2948 by hq_zhangjiangbin_tmp at 2024/04/08 end*/
	wt6670f_do_reset(chip);
	wt6670f_id = wt6670f_get_id(chip, 0x13);
	if (0x3349 == wt6670f_id) {
		g_qc3p_id = QC3P_Z350;
		qc3p_z350_init_ok = true;
		device_create_file(&client->dev, &dev_attr_qc30_device);
		gpio_direction_output(chip->reset_pin, 1);
		pr_info("[%s] is z350\n", __func__);
	...
	pr_info("[%s] firmware_version= 0x%x, chg_type = %d\n", __func__,
		firmware_version, chip->chg_type);
...
static void wt6670f_charger_shutdown(struct i2c_client *client)
{
	...
	if (g_qc3p_id == QC3P_Z350) {
		...
		wt6670f_write_byte(chip, WT6670F_REG_SOFT_RESET,
				   SOFT_RESET_VAL);
