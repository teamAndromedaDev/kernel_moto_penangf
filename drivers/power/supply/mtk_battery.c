...
static int get_battery_serialnumber(void)
{
	...
	for (i = 0; i < sizeof(bat_id_ser)/sizeof(bat_id_ser[0]); i++) {
		if(strstr(bootparams, bat_id_ser[i])) {
			break;
		}
	}
	if (i == 4) {
	...
	if (i >= sizeof(bat_id_ser)/sizeof(bat_id_ser[0])) {
		pr_err("not find bat_id, use Default Serial Number 0\n");
		i = 0;
	}
	bm_err("androidbootbattid = %d\n", i);
...
{
	return get_battery_serialnumber();
}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		if (IS_ERR_OR_NULL(bs_data->chg_psy)) {
			bs_data->chg_psy = power_supply_get_by_name("sgm4154x-charger");//devm_power_supply_get_by_phandle(&gm->gauge->pdev->dev, "charger");
...
static void mtk_battery_external_power_changed(struct power_supply *psy)
{
	...
	if (IS_ERR_OR_NULL(chg1_dev)) {
		chg1_dev = get_charger_by_name("primary_chg");
		if (chg1_dev)
			bm_err("%s, Found primary charger\n", __func__);
		else {
			bm_err("%s, *** Error : can't find primary charger ***\n"
				, __func__);
			return;
		}
	}
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		chg_psy = power_supply_get_by_name("sgm4154x-charger");  //devm_power_supply_get_by_phandle(&gm->gauge->pdev->dev,  "charger");
		bm_err("%s retry to get chg_psy\n", __func__);
		...
		ret = charger_dev_is_enabled(chg1_dev, &chg1_dev_chgen);
		...
		if (!online.intval) {
			...
			if (status.intval == POWER_SUPPLY_STATUS_NOT_CHARGING) {
				...
				if(online.intval && chg1_dev_chgen && battery_get_vbus() > 4400)
					bs_data->bat_status =
...
int battery_psy_init(struct platform_device *pdev)
{
	...
	gm->bs_data.chg_psy = power_supply_get_by_name("sgm4154x-charger"); //devm_power_supply_get_by_phandle(&pdev->dev, "charger");
