...
int mtk_hbm_brightness_set(char *name, int level)
{
	struct mt_led_data *led_dat;
	int index;
	...
	mtk_set_brightness(&led_dat->conf.cdev, level);
...
/*penang code for EKPENAN4GU-1490 by p-lizongrui at 2024/02/26 start*/
static unsigned int hbm_status;
bool get_hbm_status(void)
{
        return hbm_status;
}
EXPORT_SYMBOL(get_hbm_status);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = kstrtouint(buf, 10, &hbm_mode);
	if (ret)
		goto exit;

	if (hbm_mode == 0) {
		hbm_status = 0;
		mtk_hbm_brightness_set("lcd-backlight", 2046);
	} else {
		mtk_hbm_brightness_set("lcd-backlight", 2046);
		hbm_status = 1;
		mtk_hbm_brightness_set("lcd-backlight", 2047);
	}
	ret = size;
exit:
	return ret;
