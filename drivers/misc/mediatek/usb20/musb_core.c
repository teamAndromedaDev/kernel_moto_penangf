...
{
	/*Penang code for EKPENAN4GU-2057 by zhangjb18 at 20240312 start*/
	struct regulator *vbus = NULL;
	bool vbus_on = false;
	vbus = devm_regulator_get(glue->dev, "usb-otg-vbus");
	if (!IS_ERR_OR_NULL(vbus)) {
		vbus_on = regulator_is_enabled(vbus);
		if (vbus_on) {
			DBG(0, "%s vbus is %s, break!!!\n", __func__, vbus_on ? "on" : "off");
			return;
		}
	} else {
		DBG(0, "%s failed to get vbus\n", __func__);
	}
	/*Penang code for EKPENAN4GU-2057 by zhangjb18 at 20240312 end*/
