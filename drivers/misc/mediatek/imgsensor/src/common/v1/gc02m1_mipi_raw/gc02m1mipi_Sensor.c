...
unsigned int gc02m1_read_otp_info(unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(gc02m1_data_id);
	int index = 0;
	LOG_INF("addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
		data[index] = gc02m1_data_id[index];
	}
  	return num;
}
...
unsigned int gc02m1_read_otpdata(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	if (addr == 0x1888){
		LOG_INF("gc02m1_read_otp_info,addr=0x%x\n", addr);
		gc02m1_read_otp_info(addr, data, size);
		return size;
	}
...
EXPORT_SYMBOL(gc02m1_read_otpdata);
/*Penang Code for EKPENAN4GU-3013 by chenxiaoyong at 20240321 end*/
