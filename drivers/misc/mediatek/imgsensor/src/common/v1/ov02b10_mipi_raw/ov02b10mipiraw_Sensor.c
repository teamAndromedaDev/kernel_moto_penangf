...
unsigned int ov02b10_read_otp_info(unsigned int addr, unsigned char *data, unsigned int size)
  {
  	int num = ARRAY_SIZE(ov02b10_data_SensorID);
	int index = 0;
	LOG_INF("addr:0x%x ,size:%d",addr, size);
	for(;index<num ;index++){
		data[index] = ov02b10_data_SensorID[index];
	}
  	return num;
}
...
unsigned int ov02b10_read_otpdata(struct i2c_client *client, unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	if (addr == 0x1777){
		LOG_INF("ov02b10_read_otp_info,addr=0x%x\n", addr);
		ov02b10_read_otp_info(addr, data, size);
		return size;
	}
...
EXPORT_SYMBOL(ov02b10_read_otpdata);
/*Penang Code for EKPENAN4GU-3013 by chenxiaoyong at 20240321 end*/
#if 0
