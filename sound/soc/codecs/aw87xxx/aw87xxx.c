...
struct input_dev *aw87xx_input;
EXPORT_SYMBOL(handset_mode_flag);
EXPORT_SYMBOL(aw87xx_input);
...
void handset_monitor_select(struct input_dev *aw87xx_input,int handset_monitor_key)
{
	if (aw87xx_input) {
		input_report_key(aw87xx_input,handset_monitor_key,1);
		input_sync(aw87xx_input);
		msleep(10);
		input_report_key(aw87xx_input,handset_monitor_key,0);
		input_sync(aw87xx_input);
	} else {
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			...
			if (!strncmp(aw87xxx->current_profile, "Receiver", AW_PROFILE_STR_MAX)) {
				handset_monitor_select(aw87xx_input,HANDSET_MONITOR_ON_KEY);
				handset_mode_flag = 1;
			}
			if (strncmp(aw87xxx->current_profile, "Receiver", AW_PROFILE_STR_MAX) && (handset_mode_flag == 1)) {
				handset_monitor_select(aw87xx_input,HANDSET_MONITOR_OFF_KEY);
				handset_mode_flag = 0;
			}
		}
	}
