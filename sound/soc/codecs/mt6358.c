...
/*receiver input_key*/
extern struct input_dev *aw87xx_input;
...
int handset_monitor_init(void)
{
	...
	aw87xx_input = input_allocate_device();
	if (aw87xx_input == NULL) {
		pr_err("%s: input_allocate_device fail \n",__func__);
		return -1;
	}

	input_set_capability(aw87xx_input,EV_KEY,HANDSET_MONITOR_ON_KEY);
	input_set_capability(aw87xx_input,EV_KEY,HANDSET_MONITOR_OFF_KEY);
	aw87xx_input->name="HandsetStatus";

	ret = input_register_device(aw87xx_input);
	if(ret)
	{
		pr_err("%s: input_register_device fail \n",__func__);
		input_free_device(aw87xx_input);
		return -1;
	}
	return 0;
}
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	/* Enable MICBIAS0, MISBIAS0 = 1P9V */
	regmap_write(priv->regmap, MT6358_AUDENC_ANA_CON9, 0x0051);
	dev_err(priv->dev, "%s(), wujing enable micbias0 , 0x51\n",__func__);
...
static int mt6358_codec_probe(struct snd_soc_component *cmpnt)
{
	...
	pr_err("%s: aw87xxx_add_codec_controls  %d\n",
				__func__, ret);
	if (ret < 0) {
